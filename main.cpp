...
void crafting() {
	...
	while (less >= 5) {
		if ((int)mrand() % 100 < level_craft_make[craft_level + 1]) {
			got++;
			less -= 5;
		} else {
			less -= mrand() % 4 + 1;
...
void absorbing() {
	...
	if(now == 170ll) {
		...
		cout << "还差" << max(level_need_exp[now] - expp, 0ll) << "点经验升到下一级\n";
	}
	if (petals == "magnet" && absorb_level == 7) {
		cout << "薛皓文永远的痛\n";
		cout << "恭喜你获得 磁粉 ×" << absorb_number << "\n";
		cifen += (mrand() % 3 + 1) * absorb_number;
	}
	return;
}
...
void read() {
	std::ifstream in1("Data.ini");
	if (!in1) {
		cout << "你必须要先有一个存档！！！" << std::endl;
		return;
	}
	for (int i = 1; i <= now_petals_number; i++) {
		for (int j = 1; j <= 8; j++) {
